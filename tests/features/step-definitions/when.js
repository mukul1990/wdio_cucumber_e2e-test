...
When(/^Inventory app should (.*)\s?list (.*)$/,async function(negativeCheck,numberOfProducts){
   // if(!numberOfProducts) throw Error(`Invalid number of products provided:${numberOfProducts}`)
   var testid=undefined
   try {
     if(!numberOfProducts) throw Error(`Invalid product Count is provided:${numberOfProducts}`)
